...
window.onUserListBox = () => {
    ...
    console.log('hello');
    // setTimeout(() => {$('.showChat_inner').css('display', 'flex')}, 500);
}

window.onBackChatBox = () => {
    var my_val = $('.pcoded').attr('vertical-placement');
    if (my_val == 'right') {
        var options = {
            direction: 'left'
...
$(document).ready(function () {
    ...
    // open chat box
    // $('.displayChatbox').on('click', onDisplayChatbox);

    //open friend chat
    // $('.userlist-box').on('click', onUserListBox);
    //back to main chatbar
    // $('.back_chatBox').on('click', onBackChatBox);
    // $('.back_friendlist').on('click', onBackFriendList);
    // /*chatbar js end*/
